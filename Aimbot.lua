--[[ 
    HUYẾT v56.0: INSTANT-LOCK (RAGE MODE)
    - NEW: Nút bật/tắt Smooth.
      + ON: Kéo tâm từ từ theo tốc độ Độ/Giây.
      + OFF: Khóa cứng tâm vào mục tiêu ngay lập tức (Instant).
    - FIXED: Aimbot & ESP ổn định tuyệt đối.
    - HUD: Y=90 (Chuẩn vị trí).
]]
...
local Config = {
    Aim = {Act = false, Smooth = true, FOV = 150, Wall = true, Team = true, DegreesPerSec = 50, Part = "Head", OX = 0, OY = 0},
...
-- 3. GUI SETUP
local SG = Instance.new("ScreenGui", CoreGui); SG.Name = "v56_Huyet_Instant"; SG.IgnoreGuiInset = true
local Main = Instance.new("Frame", SG); Main.AnchorPoint = Vector2.new(0.5, 0.5); Main.Position = UDim2.new(0.5, 0, 0.5, 0); Main.Size = UDim2.new(0, 280, 0, 460); Main.BackgroundColor3 = Color3.fromRGB(15, 15, 15); Main.Visible = false; Main.Active = true; Main.Draggable = true
...
local MonTxt = Instance.new("TextLabel", Header); MonTxt.Size = UDim2.new(1, 0, 1, 0); MonTxt.BackgroundTransparency = 1; MonTxt.TextColor3 = Color3.new(1, 1, 1); MonTxt.Font = "GothamBold"; MonTxt.TextSize = 10; MonTxt.Text = "INSTANT LOCK READY"
...
-- 4. HUD (Y=90)
local HUD = Instance.new("Frame", SG); HUD.Name = "HealthHUD_v56"; HUD.Size = UDim2.new(0, 200, 0, 40); HUD.Position = UDim2.new(0.5, -95, 0, 90); HUD.BackgroundTransparency = 1
...
AddT("AIM SMOOTH", true, function(s) Config.Aim.Smooth = s end) -- Nút tắt/bật Smooth
...
RunService.RenderStepped:Connect(function(dt)
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP then
            local guiName, lineName = "v56_G_"..p.Name, "v56_L_"..p.Name
...
QA.MouseButton1Click:Connect(function() Config.Aim.Act = not Config.Aim.Act; QA.Text = "AIM: "..(Config.Aim.Act and "ON" or "OFF"); QA.BackgroundColor3 = Config.Aim.Act and Color3.fromRGB(0,150,0) or Color3.fromRGB(150,0,0) end)
